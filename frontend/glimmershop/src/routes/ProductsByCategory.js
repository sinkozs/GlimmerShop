...
function TrendingJewelry() {
  ...
  useEffect(() => {
    const fetchCategoryAndProducts = async () => {
      try {
        // Fetch category data
        const categoryResponse = await axios.get(`http://localhost:8000/categories/category-by-identifier?category_identifier=${category_name}`);
        setCategoryData(categoryResponse.data);

        // Fetch products using the category ID
        const productsResponse = await axios.get(`http://localhost:8000/categories/products-by-category/?category_id=${categoryResponse.data.category_record.id}`);
  ...
  return (
    ...
    <Container fluid className="trending-section-grid">
