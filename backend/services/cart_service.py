...
class CartService:
    ...
    async def get_user_and_cart_by_user_id(self, user_id: UUID) -> User:
        try:
            stmt = (
                select(User).options(selectinload(User.cart)).where(User.id == user_id)
            )
    ...
    async def get_cart_item(self, user_cart_id: int, product_id: int):
        try:
            stmt = select(CartItem).where(
                CartItem.cart_id == user_cart_id, CartItem.product_id == product_id
            )
    ...
    async def get_detailed_cart_from_redis(
        self, redis: aioredis.Redis, session_id: str
    ) -> List[dict]:
        try:
    ...
    async def add_new_item_to_cart(
        ...
        if user_id:
            ...
            existing_cart_item = await self.get_cart_item(
                user.cart.id, cart_item.product_id
            )
            ...
            if str(cart_item.product_id) in existing_cart_items:
                ...
                await redis.hset(
                    cart_key, str(cart_item.product_id), cart_item.quantity
                )
            ...
            if user_id:
                ...
                if str(cart_item.product_id) in existing_cart_items:
                    existing_quantity = int(
                        existing_cart_items[str(cart_item.product_id)]
                    )
